...
from CT_Al_pdk.technology.metal import METAL
...
class _Vias(fpt.IViasFactory):
    ...
    def __call__(self, curve: fpt.ICurve, length: float) -> Tuple[fpt.IElement, Tuple[float, float]]:
        from CT_Al_pdk.components.via.via import Via
        from CT_Al_pdk.components.via.vias import Vias
...
def vias1(end_types: Tuple[fpt.IMetalLineType, fpt.IMetalLineType]) -> fpt.IViasFactory:
    ...
    return _Vias(
        initial_type=a,
        final_type=b,
        top_layer=TECH.LAYER.Top_Metal,
        via_layer=TECH.LAYER.Top_Via,
        bottom_layer=TECH.LAYER.first_thick_metal,
    )
...
class AUTO_VIAS:
    ...
    def DEFAULT(cls):
        return fpt.AutoVias().updated(
            [
                (METAL.Top_Metal >> METAL.first_thick_metal, vias1),
