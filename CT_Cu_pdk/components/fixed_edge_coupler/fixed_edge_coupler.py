...
class Fixed_Edge_Coupler(fp.PCell):
    ...
    def build(self) -> Tuple[fp.InstanceSet, fp.ElementSet, fp.PortSet]:
        ...
        wg = TECH.WG.Hard_Mask_WG.C.WIRE(curve=fp.g.Line(length=199, anchor=fp.Anchor.END, origin=(0, 0)))
        insts += wg
        ports += wg["op_1"].with_name(self.port_names[0])

        polygon2 = fp.el.Polygon([(-250, -50), (-200, -50), (-200, 50), (-250, 50)], layer=TECH.LAYER.Deep_Trench)
        elems += polygon2
