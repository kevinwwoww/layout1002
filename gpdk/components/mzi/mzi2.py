...
class MZI(fp.PCell):
    ...
    coupler_spacing: float = fp.PositiveFloatParam(default=1)
    ...
    dc_left: fp.IDevice = fp.DeviceParam(type=DirectionalCouplerBend, port_count=4, required=False)
    dc_right: fp.IDevice = fp.DeviceParam(type=DirectionalCouplerBend, port_count=4, required=False)

    def _default_waveguide_type(self):
        return get_technology().WG.FWG.C.WIRE

    def _default_dc_left(self):
        return DirectionalCouplerBend(
            name="dc",
            coupler_spacing=self.coupler_spacing,
            coupler_length=self.coupler_length,
            bend_radius=self.bend_radius,
            straight_after_bend=self.straight_after_bend,
            waveguide_type=get_technology().WG.FWG.C.WIRE
        )
    ...
    def build(self) -> Tuple[fp.InstanceSet, fp.ElementSet, fp.PortSet]:
        ...
        link_up = fp.create_links(
            link_type=get_technology().WG.FWG.C.EXPANDED,
            bend_factory=get_technology().WG.FWG.C.WIRE.BEND_CIRCULAR,
            specs=[
