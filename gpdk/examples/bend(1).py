...
class _GenericResolutionCurve(CurveMixin, fp.ICurve):
    ...
    def curve_points(self) -> Tuple[fp.Point2D, ...]:
        ...
        # print(f"raw_curve_point: {self.raw_curve_points[0]} => {self.raw_curve_points[1]}, type: {type(self)}")
        # print(f"curve_point: {points[0]} => {points[1]}, type: {type(self)}")
...
route = fp.LinkSmooth(
    ...
    bend_factory=TaperedBendFactory(min_bend_radius=8.0, waveguide_type=TECH.WG.FWG.C.WIRE),
)
