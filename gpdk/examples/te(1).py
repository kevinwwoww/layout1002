    ...
    def raw_curve_points(self) -> Tuple[Point2D, ...]:
        ...
        if self.taper_length > 0:
            ...
            angles_middle = np.linspace(
                angles_start[-1] + np.degrees(length_res / self.min_bend_radius),
                angles_stop[0] - np.degrees(length_res / self.min_bend_radius), n)
        ...
        # return self.transform(points.tolist())
...
class TaperedBendFactory(fpt.IBendWaveguideFactory):
    ...
    def __call__(self, central_angle: float):
        ...
        # print(bend.raw_curve.radius_eff)
        # fp.plot(bend)
...
route = fp.LinkSmooth(
    ...
    bend_factory=TaperedBendFactory(min_bend_radius=20.0, waveguide_type=TECH.WG.FWG.C.WIRE)
)

fp.plot(route)

bend = EulerBend()
print(bend.ports[0].position)
print(gpdk.util.port_util.get_right_ports(bend))
