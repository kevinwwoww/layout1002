...
# every dataclass pcell must be decorated by @dataclass(eq=False) means fallback to inherited __eq__ and __hash__, hashable is required because runtime use cache heavily
@dataclass(eq=False)
class RingResonator(fp.PCell, band="C"):
    ...
    def build(self):
        ...
        ring = ring_type(curve=fp.g.EllipticalArc(radius=self.ring_radius)).with_ports((None, None)).with_name("ring")
        insts += ring
...
class RingResonator2(RingResonator, band="C"):
    ring_radius: float = fp.PositiveFloatParam(default=10, locked=True)  # `final` = True makes `ring_radius` final
...
@dataclass(eq=False)
class RingResonator3(RingResonator2, locked=True, band="C"):  # `final` = True makes class `RingResonator3` final
    computed_value: float = field(init=False)
    computed_v2: float = 14
...
if __name__ == "__main__":
    ...
    # fixed template end
    # custom region start
    r0 = RingResonator(ring_type=TECH.WG.FWG.C.WIRE, top_type=TECH.WG.FWG.C.WIRE, bottom_type=TECH.WG.MWG.C.WIRE)
    library += r0
    ...
    library += r2
    r3 = RingResonator3().translated(80, 0)
    library += r3
    # custom region end
