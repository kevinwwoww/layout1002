...
class CircularBendFactory(fpt.IBendWaveguideFactory):
    ...
    def __call__(self, central_angle: float):
        from CT_pCu_pdk.components.bend.bend_circular import BendCircular, BendCircular90, BendCircular90_Strip_WG_C_WIRE
        ...
        if fp.is_close(abs(central_angle), math.pi / 2):
            if self.waveguide_type == TECH.WG.Strip.C.WIRE:
...
class EulerBendFactory(fpt.IBendWaveguideFactory):
    ...
    def __call__(self, central_angle: float):
        from CT_pCu_pdk.components.bend.bend_euler import BendEuler, BendEuler90, BendEuler90_Strip_WG_C_WIRE
        from CT_pCu_pdk.technology.interfaces import CoreWaveguideType
        from CT_pCu_pdk.technology import get_technology
        ...
        bend = None
        if fp.is_close(abs(central_angle), math.pi / 2):
            if self.waveguide_type == TECH.WG.Hard_Mask_WG.C.WIRE:
