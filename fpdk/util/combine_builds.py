...
def combine_builds(name: str, specs: Iterable[Dict[str, Any]]) -> fp.IDevice:
    ...
    for spec in specs:
        ...
        if instance is not None:
            ...
            translated_instance = instance.translated(*origin)
            result.append(translated_instance)
        else:
        ...
        rect = fp.el.Rect(width=width, height=height, center=(tx, ty), layer=LAYER.DEVREC_NOTE)
        label = fp.el.Label(spec["label"], baseline=fp.TextBaseline.BOTTOM, layer=LAYER.DEVREC_NOTE)
